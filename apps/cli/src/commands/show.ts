...
export async function executeShow(
  ...
  let task;
  try {
    task = await client.getTask(numId);
  } catch (e) {
    return exitFromError(e);
  }
...
export function registerShow(program: Command): void {
  program
    .command('show <numId>')
    .description('Show one task by numeric id.')
    .option('--json', 'Output as JSON')
    .action(async (numId: string, opts: { json?: boolean }) => {
      const code = await executeShow(numId, opts);
      process.exit(code);
    });
}
